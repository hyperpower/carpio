...
namespace carpio {
    ...
    void test_adaptive_find(){
        ...
        Node<Float, Float, 3>* cn = GetpNodeAt(&g, 0.3, 0.3, 0.7);
        ...
        Node<Float, Float, 3> *nei = cn->get_neighbor(cn, ToDirection(_P_, _P_, _P_));
        vtkSmartPointer<vtkActor> actornei = vtk_new_actor(nei);
        actornei->GetProperty()->SetColor(1.0 / 4.0, 0.0, 0.0); //(R,G,B)

...
template<class BOX>
inline bool do_intersect(BOX* pb, Triangle<typename BOX::value_type, BOX::Dim>* pt)
{
...
bool do_intersect_box_obj(BOX* pb, utPointer upt, OBJ_TYPE type)
{
	...
	if (type == TRIANGLE) {
		Triangle<typename BOX::value_type, BOX::Dim>* pt = (Triangle<typename BOX::value_type,
				BOX::Dim>*) upt;

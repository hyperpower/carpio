...
public:
	template<class Args>
	void traversal_pre_order(std::function<void(pNode, Args)> fun, Args &args) {
		this->_preorder(_root, fun, args);
	}
...
void BBTree<BOX>::PreOrder_flag(Int& flag, pFun_BBTree_flag visit,
		utPointer utp) {
	_preorder_flag(flag, _root, visit, utp);
}
...
void BBTree<BOX>::output_vtk_height(const String& fn, Int hei) {
	...
	for (auto iter = lbn.begin(); iter != lbn.end(); ++iter) {
		for (uInt i = 0; i < NUM_VERTEXES; i++) {
			fprintf(data, "%f %f %f \n",
					(*iter)->box.get_point(_ORDER_VTK[i][0], _ORDER_VTK[i][1],
							_ORDER_VTK[i][2]).x(),
					(*iter)->box.get_point(_ORDER_VTK[i][0], _ORDER_VTK[i][1],
							_ORDER_VTK[i][2]).y(),
					BOX::Dim == 3 ?
							(*iter)->box.get_point(_ORDER_VTK[i][0],

...
public:
	typedef AABBox<TYPE, DIM> self_class;
...
public:
	...
	AABBox(const self_class& a, const self_class& b) {
		self_class& self = (*this);
		for (st i = 0; i < DIM; ++i) {
	...
	self_class& operator=(const self_class& a) {
		if (this == &a) {
			return *this;
		}
		self_class& self = (*this);
	...
	bool operator<(const self_class& rhs) const {
		//compare center point x -> y -> z -> point address
		const self_class& self = (*this);
	...
	bool are_overlapping(AABBox<TYPE, DIM>* other) const;
...
void AABBox<TYPE, DIM>::reset(OBJ_TYPE t, utPointer bounded, Float x1, Float y1,
	...
	_return_if_fail(x2 >= x1 && y2 >= y1 && z2 >= z1);
	type = t;
	self_class& self = (*this);
	self[Min][X] = x1;
	self[Min][Y] = y1;
...
template<class TYPE, st DIM>
void AABBox<TYPE, DIM>::reset(OBJ_TYPE t, pTri pt) {
	self_class& self = (*this);
...
template<class TYPE, st DIM>
bool AABBox<TYPE, DIM>::are_overlapping(AABBox<TYPE, DIM>* other) const {
